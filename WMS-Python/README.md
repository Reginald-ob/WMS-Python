...
使用 `PyInstaller` 將專案打包為 Windows 可執行的 `.exe` 檔案。

### 1. 準備工作

確保已安裝 PyInstaller，並確認專案可在本地正常運行。

### 2. 執行打包指令

在專案根目錄執行以下指令。注意我們需要使用 `--add-data` 將 `schema.sql` 包含進去，否則初始化會失敗。

```bash
# Windows (PowerShell)
pyinstaller --noconsole --onefile --name "WMS-System" `
    --add-data "src/infrastructure/database/schema.sql;src/infrastructure/database" `
    src/main.py
...
* `--add-data`: 格式為 `來源路徑;目標路徑`。

### 3. 執行

打包完成後，執行檔將位於 `dist/WMS-System.exe`。
*注意：首次執行時，程式會在 `.exe` 同級目錄下自動產生 `wms.db` 資料庫檔案。*
