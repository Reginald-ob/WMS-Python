...
class InventoryService:
    ...
    def search_products(self, keyword: str) -> List[Product]:
        """搜尋商品 (Pass-through to Repository)"""
        return self.product_repo.search_products(keyword)
    
    # --- 變體管理 (Variant Management) ---

    def update_variant(self, variant: Variant) -> None:
        """
        更新變體的基本資料 (Metadata)，如 SKU、尺寸、顏色、安全庫存等。
        
        保護機制：
        此方法會忽略傳入物件中的 `stock_qty`，強制使用資料庫目前的庫存值。
        庫存數量變動必須嚴格透過單據 (Inbound/Outbound/Adjust) 進行。

        Args:
            variant (Variant): 包含更新後資料的變體物件 (必須包含 ID)

        Raises:
            EntityNotFoundError: 當變體 ID 不存在時
            BusinessRuleViolation: 當變體沒有 ID 時
        """
        if variant.id is None:
            raise BusinessRuleViolation("更新變體時必須提供 ID")

        # 1. 檢查變體是否存在，並取得當前真實庫存
        existing_variant = self.product_repo.get_variant_by_id(variant.id)
        if not existing_variant:
            raise EntityNotFoundError(f"變體 ID {variant.id} 不存在")

        # 2. 庫存保護機制 (Stock Protection)
        # 關鍵邏輯：無論傳入什麼 stock_qty，都強制覆蓋回 DB 裡的原值
        # 這確保了 "庫存 = 交易計算結果" 的原則不被破壞
        if variant.stock_qty != existing_variant.stock_qty:
            logger.warning(
                f"偵測到嘗試直接修改庫存行為 (ID: {variant.id})。"
                f"輸入: {variant.stock_qty}, 原值: {existing_variant.stock_qty}。"
                f"系統已自動忽略此數值，維持原庫存。"
            )
        
        variant.stock_qty = existing_variant.stock_qty

        # 3. 呼叫 Repository 進行持久化
        self.product_repo.update_variant(variant)
        logger.info(f"變體資料更新成功: {variant.display_name} (ID: {variant.id})")
